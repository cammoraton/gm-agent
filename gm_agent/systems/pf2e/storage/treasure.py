...
class TreasureStore:
    ...
    def __init__(
        ...
        treasure_table: dict[int, dict[str, float]] | None = None,
    ):
        self.campaign_id = campaign_id
        self.base_dir = base_dir or CAMPAIGNS_DIR
        self.db_path = self.base_dir / campaign_id / "treasure.db"
        self._conn: sqlite3.Connection | None = None
        # Use injected table (from DB) or fall back to module-level constant
        self._treasure_table = treasure_table if treasure_table is not None else TREASURE_BY_LEVEL
        self._ensure_schema()
    ...
    def get_wealth_by_level(self, party_level: int, party_size: int = 4) -> dict[str, Any]:
        ...
        expected = self._treasure_table.get(party_level, {"total": 0, "currency": 0})
