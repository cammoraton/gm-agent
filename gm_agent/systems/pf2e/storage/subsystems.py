...
class SubsystemInstance(BaseModel):
    ...
    id: str
    campaign_id: str
    subsystem_type: str  # "vp", "influence", "research", "chase", "infiltration", "hazard", "haunt"
    name: str
    ...
    # Per-condition disable progress: maps str(condition_index) -> successes achieved
    disable_conditions_progress: dict[str, int] = Field(default_factory=dict)
    ...
    edge_points: int = 0  # infiltration edge points (separate from awareness)
...
class SubsystemStore:
    ...
    def create(
        ...
        Args:
            subsystem_type: Type of subsystem (vp, influence, research, chase, infiltration, hazard, haunt).
    ...
    def _init_from_config(self, instance: SubsystemInstance, config: dict[str, Any]) -> None:
        ...
        if st in ("vp", "influence", "research"):
            ...
            if st == "influence":
                # Parse rich NPC config: {approach_dcs, weaknesses, resistances, min_vp, max_vp}
                npcs = config.get("npcs", {})
                instance.config["npcs"] = npcs
                # Initialize VP tracking for each NPC
                for npc_name, npc_cfg in npcs.items():
                    if npc_name not in instance.victory_points:
                        instance.victory_points[npc_name] = npc_cfg.get("min_vp", 0)
                        instance.thresholds[npc_name] = {
                            "minor": npc_cfg.get("min_vp", 0) + 2,
                            "major": npc_cfg.get("max_vp", 5),
                        }

            elif st == "research":
                # Parse sources and vp_target
                sources = config.get("sources", [])
                instance.config["sources"] = sources
                instance.config["vp_target"] = config.get("vp_target", 0)
                instance.config["source_contributions"] = {}
            ...
            # Also store in config for richer access
            instance.config["obstacles"] = instance.obstacles
            ...
            # Haunt metadata defaults
            if "spiritual_immunity" not in config:
                instance.config["spiritual_immunity"] = True
            if "reset_interval" not in config:
                instance.config["reset_interval"] = "1 day"

        elif st == "infiltration":
            instance.detection_threshold = config.get("detection_threshold", 10)
            # Parse edge benefits: [{cost, description}]
            instance.config["edge_benefits"] = config.get("edge_benefits", [])
