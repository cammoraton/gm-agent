...
class EncounterServer(MCPServer):
    ...
    @staticmethod
    def _try_load_tables_from_db(db_path: str) -> None:
        """Load XP tables from DB (v7+) into module-level constants.

        Gracefully no-ops if the DB has no pf2e_creation_table rows.
        Updates CREATURE_XP_TABLE, SIMPLE_HAZARD_XP_TABLE, THREAT_THRESHOLDS,
        XP_PER_EXTRA_PLAYER at module level so all pure functions use DB values.
        """
        try:
            from gm_agent.rag import PathfinderSearch  # pylint: disable=import-outside-toplevel

            search = PathfinderSearch(db_path=db_path)
            xp_table = search.get_creation_table("creature_xp")
            if xp_table:
                CREATURE_XP_TABLE.clear()
                CREATURE_XP_TABLE.update(xp_table)

            hz_table = search.get_creation_table("simple_hazard_xp")
            if hz_table:
                SIMPLE_HAZARD_XP_TABLE.clear()
                SIMPLE_HAZARD_XP_TABLE.update(hz_table)

            thresholds = search.get_creation_table("threat_thresholds")
            if thresholds:
                THREAT_THRESHOLDS.clear()
                THREAT_THRESHOLDS.update(thresholds)

            xp_extra = search.get_creation_table("xp_per_extra_player")
            if xp_extra:
                XP_PER_EXTRA_PLAYER.clear()
                XP_PER_EXTRA_PLAYER.update(xp_extra)
        except Exception:  # pylint: disable=broad-except
            pass  # Any error → keep module-level fallback constants
