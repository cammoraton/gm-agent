...
if TYPE_CHECKING:
    from .base import SystemToolPlugin
...
class CampaignStateServer(MCPServer):
    ...
    Game-system-specific tools are injected via system_plugins.
    """

    def __init__(self, campaign_id: str, system_plugins: list[SystemToolPlugin] | None = None):
        self.campaign_id = campaign_id
        self._system_plugins = system_plugins or []
    ...
    def _build_tools(self) -> list[ToolDef]:
        ...
        tools = [
            ...
            # Fiction extraction tool
            ToolDef(
                name="extract_fiction_knowledge",
                description="Extract knowledge from a completed fiction tree (Microscope, Ex Novo, Delve, Ex Umbra) "
                "into campaign party knowledge. Idempotent — safe to re-run.",
                parameters=[
                    ToolParameter(
                        name="root_node_id",
                        type="string",
                        description="Fiction tree root node ID to extract from.",
                    ),
                ],
            ),
            # Session recap tool
        ...
        # Append tools from system plugins
        for plugin in self._system_plugins:
            tools.extend(plugin.tool_defs())
    ...
    def call_tool(self, name: str, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            if name == "update_scene":
            ...
            elif name == "extract_fiction_knowledge":
                return self._extract_fiction_knowledge(args)
                ...
                for plugin in self._system_plugins:
                    try:
                        return plugin.call_tool(name, args, self)
                    except KeyError:
                        continue
    ...
    # PF2e-specific methods (calculate_travel_time, check_hazard_detection,
    # ap_progress, treasure) have been moved to PF2eCampaignToolPlugin.

    def _extract_fiction_knowledge(self, args: dict[str, Any]) -> ToolResult:
        """Extract knowledge from a fiction tree into campaign knowledge."""
        from ..storage.fiction_tree import FictionTreeStore
        from ..prep.fiction_extraction import extract_fiction_knowledge

        root_node_id = args.get("root_node_id", "")
        if not root_node_id:
            return ToolResult(success=False, error="root_node_id is required")

        fiction_store = FictionTreeStore(self.campaign_id, base_dir=CAMPAIGNS_DIR)
        try:
            count = extract_fiction_knowledge(fiction_store, self.knowledge, root_node_id)
        finally:
            fiction_store.close()
        ...
        if count == 0:
            return ToolResult(
                success=True,
                data="No new knowledge extracted (already up to date or node not found).",
            )

        return ToolResult(
            success=True,
            data=f"Extracted {count} knowledge entries from fiction tree into campaign knowledge.",
        )
